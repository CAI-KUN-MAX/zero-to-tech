...
var parseInlineTransforms = (target, propName, animationInlineStyles) => {
  ...
  if (inlineTransforms) {
    ...
    while (pos < len) {
      while (pos < len && inlineTransforms.charCodeAt(pos) === 32)
        pos++;
      if (pos >= len)
        break;
      const nameStart = pos;
      while (pos < len && inlineTransforms.charCodeAt(pos) !== 40)
        pos++;
      if (pos >= len)
        break;
    ...
    if (propName === "translate3d" && fullTranslateValue) {
      if (animationInlineStyles)
        animationInlineStyles[propName] = fullTranslateValue;
      return fullTranslateValue;
    }
    const cached = cachedTransforms[propName];
    if (!isUnd(cached)) {
      if (animationInlineStyles)
        animationInlineStyles[propName] = cached;
      return cached;
    }
  }
...
var buildTransformString = (props) => {
  ...
  for (let i = 0, l = validTransforms.length; i < l; i++) {
    ...
    if (key2 === "rotateZ") {
      if (props.rotate3d !== void 0)
        str += `rotate3d(${props.rotate3d}) `;
    }
  }
  if (props.matrix !== void 0)
    str += `matrix(${props.matrix}) `;
  if (props.matrix3d !== void 0)
    str += `matrix3d(${props.matrix3d}) `;
  return str;
};
    ...
    for (let i = 0; i < al; i++) {
      ...
      for (let j = 0, m = tas.length; j < m; j++) {
        ...
        if (ta.detect(target)) {
          ...
          if (entry && (!entry.gate || entry.gate(target)))
            return entry;
          break outer;
        }
      }
...
var resolveCssVar = (value, target) => {
  ...
  if ((!computed || computed.trim() === emptyString) && match[2])
    computed = match[2].trim();
  return computed || 0;
};
...
var getOriginalAnimatableValue = (target, propName, tweenType, animationInlineStyles) => {
  ...
  if (adapterProp) {
    ...
    if (value && animationInlineStyles)
      animationInlineStyles[propName] = value;
...
var tick = (tickable, time, muteCallbacks, internalRender, tickMode) => {
  ...
  if (tickable._hasChildren) {
    ...
    forEachChildren(tl, (child) => {
      ...
      if (tlIsRunningBackwards && childTime > child._delay + child.duration)
        return;
...
var Clock = class {
  ...
  set fps(frameRate) {
    ...
    if (fps > defaults.frameRate)
      defaults.frameRate = fps;
    this._fps = fps;
  ...
  requestTick(time) {
    ...
    if (elapsed + tolerance < frameDuration)
...
var Timeline = class extends Timer {
  ...
  sync(synced, position) {
    ...
    if (editor)
      editor.addTimelineChild = childHook;
    return result;
  }
  /**
   * @param  {TargetsParam} targets
  ...
  call(callback, position) {
    if (isUnd(callback) || callback && !isFnc(callback))
      return this;
    if (globals.editor && globals.editor.addTimelineCall)
      position = globals.editor.addTimelineCall(callback, position, this.id);
  ...
  label(labelName, position) {
    if (isUnd(labelName) || labelName && !isStr(labelName))
      return this;
    if (globals.editor && globals.editor.addTimelineLabel)
      position = globals.editor.addTimelineLabel(labelName, position, this.id);
...
var muteNodeTransition = (node) => {
  ...
  if ($el && !store.has($el))
    store.set($el, muteElementTransition($el));
  if ($measure && !store.has($measure))
    store.set($measure, muteElementTransition($measure));
};
...
var detachNode = (node) => {
  ...
  if (!parent)
    return;
  if (parent._head === node)
    parent._head = node._next;
  if (parent._tail === node)
    parent._tail = node._prev;
  if (node._prev)
...
var createNode = ($el, parentNode, state, recycledNode) => {
  ...
  if (!dataId)
    dataId = $el.dataset.layoutId = `node-${nodeId++}`;
...
var recordNodeState = (node, $measure, computedStyle, skipMeasurements) => {
  ...
  if (isRoot)
    node.layout.absoluteCoords = position === "fixed" || position === "absolute";
  ...
  if (node.hasTransform && !skipMeasurements) {
    ...
    if (!transitionMuteStore.get($el))
      node.inlineTransition = muteElementTransition($el);
    if ($measure === $el) {
      $el.style.transform = "none";
    } else {
  ...
  for (let name in properties) {
    ...
    if (!isUnd(computedProp))
      properties[name] = computedProp;
  }
...
var LayoutSnapshot = class {
  ...
  registerElement($el, parentNode) {
    ...
    while (stack.length) {
      ...
      if (!$current || $current.nodeType !== 1 || isSvg($current))
        continue;
...
var AutoLayout = class {
  ...
  constructor(root, params = {}) {
    ...
    if (swapAtSplitParams[0])
      for (let name in swapAtSplitParams[0])
        this.properties.add(name);
    if (enterFromSplitParams[0])
      for (let name in enterFromSplitParams[0])
        this.properties.add(name);
    if (leaveToSplitParams[0])
      for (let name in leaveToSplitParams[0])
        this.properties.add(name);
  ...
  return (target, i, t, _, tl) => {
    ...
    if (!values.length) {
      ...
      for (let k = 1; k < total; k++)
        if (values[k] > maxValue2)
          maxValue2 = values[k];
      if (staggerEase || reversed) {
        for (let k = 0; k < total; k++) {
          let v = values[k];
          if (staggerEase)
            v = staggerEase(v / maxValue2) * maxValue2;
          if (reversed)
            v = axis ? -v : abs(maxValue2 - v);
          values[k] = v;
        }
      }
      if (hasJitter) {
...
var scrambleText = (params = {}) => {
  ...
  return (target, index, targets, prevTween) => {
    ...
    if (resolvedFrom === "random") {
      ...
      for (let i = 0; i < animLength; i++)
        charOrder[indices[i]] = i;
    }
    if (reversed) {
      const last = animLength - 1;
      for (let i = 0; i < animLength; i++)
        charOrder[i] = last - charOrder[i];
    }
    ...
    if (endLength < animLength && resolvedFrom !== "left" && resolvedFrom !== "right" && resolvedFrom !== "random") {
      ...
      for (let c = endLength; c < animLength; c++) {
        if (charEnds[c] > maxExtraEnd)
          maxExtraEnd = charEnds[c];
      }
      const targets2 = new Array(endLength);
      for (let c = 0; c < endLength; c++)
        targets2[c] = c;
